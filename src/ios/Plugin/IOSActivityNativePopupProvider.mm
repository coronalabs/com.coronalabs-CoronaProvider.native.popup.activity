...
IOSActivityNativePopupProvider::showPopup( lua_State *L )
{
	...
	if ( context )
	{
		...
		if ( lua_istable( L, 2 ) )
		{
			...
			static CoronaLuaRef listenerRef = NULL;
			lua_getfield( L, -1, "listener" );
			if ( CoronaLuaIsListener( L, -1, kEventName ) )
			{
				listenerRef = CoronaLuaNewRef( L, -1 );
			}
			lua_pop( L, 1 );
			...
			if ( listenerRef )
			{
				handler = ^(NSString *activityType, BOOL completed, NSArray *returnedItems, NSError *activityError)
				{
                    ...
                    CoronaLuaDispatchEvent(L, listenerRef, 0);
	...
	unsigned int direction )
{
	...
	else
	{
		...
		else
		{
            ...
            id<CoronaRuntime> runtime = (__bridge id<CoronaRuntime>)CoronaLuaGetContext( L );
