...
UIActivityForLuaStringMapping()
{
	...
	if ( ! sMapping )
	{
        ...
        if (@available(iOS 9.0, *)) {
            // Activity types introduced in iOS 9
            if ( nil != UIActivityTypeOpenInIBooks )
            {
        ...
        if (@available(iOS 11.0, *)) {
            // Activity types introduced in iOS 11
            if ( nil != UIActivityTypeMarkupAsPDF )
            {
                [mapping addEntriesFromDictionary:@{
                    @"UIActivityTypeMarkupAsPDF": UIActivityTypeMarkupAsPDF
                }];
            }
        }
